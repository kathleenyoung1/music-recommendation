...
def main(spark):
    ...
    # assess the best model on training data:
    ...
    print('***test on best model so far***')
    print("rmse: ", rmse, "it took ", endnow - startnow)
    ...
    for k in maxIters:
        ...
        als = ALS(maxIter=k, regParam=0.001,rank = 100, alpha = , numUserBlocks = 100, numItemBlocks = 100, userCol="nuser_id", itemCol="ntrack_id", ratingCol="count", coldStartStrategy="drop")
        endnow = time.time()
    ...
    #val_rec.show()
        ...
        ranks = spark.sparkContext.parallelize(rankLists)
        print('***TEST***')
        print("maxIters used: ", k, "rmse: ", rmse)
        metrics = RankingMetrics(ranks)
        mavgpre = metrics.meanAveragePrecision
        print(f"map = {mavgpre}")
        preck = metrics.precisionAt(500)
